...
dat <- read.csv("//141.20.141.12/SAN_BioGeo/_SHARED_DATA/FP_TaKa/Harmony/600_extracts_sep22.csv")
...
# If you add in new data, rechedk the rows and add row numbers of those datasets with values for 2020 (d20)or 2010 (d10).
...
q1=qq %>% 
  ...
  filter(#Accuracy == 'Overall accuracy',
    Observer == 'Mean between Tamanna & Florian',
    Dataset %in% c('CCI_2020_ws (binary)',
                   'CCI_2020_wos (binary)',
                   'Hansen_2020',
                   'GLobeland_2020_ws',
                   'MODIS_PNTV_2020',
...
q1 %>% 
  # filter(Percent == '15') %>% 
  filter(Dataset %in% c('CCI_2020_ws (binary)',
                        'CCI_2020_wos (binary)',
                        'Hansen_2020',
                        'GLobeland_2020_ws',
                        'Moulds_2010',
  ...
  ggtitle('Indian TDF dataset comparison using mean of estimates from two observers') + labs(x ='Classification threshold in % (classified as forest if TC of dataset or visual interpretation equal or above threshold)', y= 'Performance in %') 
