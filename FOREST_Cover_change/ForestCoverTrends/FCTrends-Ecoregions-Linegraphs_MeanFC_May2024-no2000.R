...
#Purpose: This code is used to calculate absolute percentage change in forest cover over time for different eco-regions. 
#Resolution of dataset: 300m x 300m 
#Years: all but no 2000
...
stacked = stack(paste0("//141.20.141.12/SAN_BioGeo/_PERSONAL_FOLDERS/kalamtam/GEODATA/4-ReProcessed_Datasets_ForAnalysis/1-Base_Stacks_Final/2024/2024/No-Moulds/IndiaTDF_1880-1930-1975-1985-1995-2000-2010-2020_Ecoregions_300m_7755_noMoulds.tif"))
...
stack_df <- stack_df %>%
  dplyr::select(-EN2000) %>% # Drop the column EN2000
  mutate(
...
stack_all <- stack_df %>%
  dplyr::select(1:16) %>% #selecting Study area:EN2020, or all columns with FC values
...
write.xlsx(Final, file = "//141.20.141.12/SAN_BioGeo/_PERSONAL_FOLDERS/kalamtam/CHAPTERS/Chapter 01/3_FinalAnalysis-April2024/2_AbsoluteChange-300m/With CCI-no moulds/AbsoluteChange_5TC_nomoulds_300m_Ecoregions.xlsx", rowNames = FALSE)
...
selected_years<-c(1880, 1930,1975,1985,1995,2010, 2020)
