...
stack_df_filter_sub <- new_stack_df[, c(2:6,8:9)] 
...
soms_func = function(som_data){
  vars = c(1:7)   #Changes based on bands selected-- [check number of columns after selecting bands-- View(stack_matrix), leave out X and Y columns for this part]
  dat = som_data[[1]]
  dim1 = som_data[[2]]
  dim2 = som_data[[3]]
  nam = som_data[[4]]
...
postproc_function = function(som_res){
  ...
  block = as.data.frame(matrix(unlist(dat$data),nrow = nrow(dat$data[[1]]), ncol = 7))  #nocol changes based on number of bands selected. n=3 if only 3 bands, 14 if all 14 bands
...
outFolder <- ("//141.20.141.12/SAN_BioGeo/_PERSONAL_FOLDERS/kalamtam/CHAPTERS/Chapter 01/3_FinalAnalysis-April2024/3_SOMS/noMoulds/")
...
SOM_FC_df <- SOM_df %>% right_join(new_stack_df, by=c("x", "y")) %>% dplyr::select(x, y,cluster,RD1880,RD1930,RD1975,RD1985,RD1995,EN2010,EN2020)
...
for (cluster_num in cluster_numbers) {
  ...
  cluster_data <- cluster_data[, c("ID", "RD1880","RD1930","RD1975","RD1985","RD1995","EN2010","EN2020")]
  ...
  plot <- ggplot() +
    ...
    scale_x_continuous(breaks = c(1880, 1930, 1975, 1985, 1995, 2010, 2020), limits = c(1880, 2020)) +
...
combined_plot
ggsave(paste0(outFolder, "3Clusters_trajec_300m_5TC_Nomoulds_no2000.png"), combined_plot, width = 12, height = 6, units = "in")
...
writeRaster(temp_raster,filename=paste0(outFolder, "6Clusters5TC_nomoulds-no2000_3km.tif"),format="GTiff",overwrite=T,progress="text")
...
# Now, create the trajectorz plots
cluster_numbers <- 1:10 # Define a vector of cluster numbers --> changes based on total number of clusters.
plot_list <- list() # Create an empty list to store the plots
colors <- c("#440154", "#21918c", "#90d743","#fde725", "#31688e","#46327e","#6ece58")
