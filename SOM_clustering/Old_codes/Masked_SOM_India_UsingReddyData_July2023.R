...
stack_df <- stack_df %>% select(-RY1985) %>% 
...
stack_df_filter <- stack_df %>% filter(StudyArea == 1 & !is.na(RD1880)) %>% 
  ...
  #mutate(X1930_X2000 = ifelse(X1930_X2000 > 100, 100, X1930_X2000)) %>%
  #mutate(X1930_X2000 = ifelse(X1930_X2000 < 0, 0, X1930_X2000)) %>%
...
stack_df_filter_sub <- stack_df_filter[, c(8:15)] ##leave out all columns except En2020, absolute % change (x2010_2020,X2000_2010..),FCC_Avg, MaxGain & MaxLoss 
stack_matrix = as.matrix(stack_df_filter_sub)

#clust_num3 <- NbClust(data = stack_df_filter_sub,
...
som.perf.df = as.data.frame(matrix(ncol=8,nrow=2)) #NROW NUMBER CHANGES BASED ON NUMBER OF CLUSTER COMBINATIONS (if only one cluster combo 2x2=1, if two, 3x3, 3x1= nrow=2)
colnames(som.perf.df) <- c("name","cluster_combo","clus","DB","mean","sd","hom","var")
som.perf.df[,2] <- c("3x1","3x2") #ADD IN CLUSTER COMBINATION YOU WANT HERE
som.perf.df[,3] <- c(3,6)     #ADD IN CLUSTER COMBINATION VALUE HERE
...
for(i in c(1,2)){    #VALUE CHANGES BASED ON TOTAL NUMBER OF CLUSTER COMBINATIONS e.g. 4 cluster combinations
  ...
  png(paste0('//141.20.141.12/SAN_BioGeo/_PERSONAL_FOLDERS/kalamtam/CHAPTERS/Chapter 01/GIS AND MAPPING/ANALYSIS 2023/SOM_clusters/Clustering_May2023/With_Reddy/SOMS_MS_3-6clust/',"_performanceplot",as.character(somSelect),".png"),height=3000,width=3000,res=500)
  par(mar = c(5, 4, 4, 5))
  plot(som.perf.df[c(1:2),3],som.perf.df[c(1:2),5],pch=2,col=4,  #VALUE CHANGES IN C() BASED ON TOTAL NUMBER OF CLUSTER COMBINATIONS 
       ylim=range(pretty(c(0,max(som.perf.df[c(1:2),5])))),      #VALUE CHANGES IN C() BASED ON TOTAL NUMBER OF CLUSTER COMBINATIONS
       ylab="Mean distance to cluster centroid",xlab="cluster size")
  lines(som.perf.df[c(1:2),3],som.perf.df[c(1:2),5],col=4)         #VALUE CHANGES IN C() BASED ON TOTAL NUMBER OF CLUSTER COMBINATIONS
  par(new=T)
  plot(som.perf.df[c(1:2),3],som.perf.df[c(1:2),4],type="p",axes=F,bty="n",xlab="",ylab="",pch=3,col=2, #VALUE CHANGES IN C() BASED ON TOTAL NUMBER OF CLUSTER COMBINATIONS
       ylim=range(pretty(c(0,3))))
  lines(som.perf.df[c(1:2),3],som.perf.df[c(1:2),4],col=2) #VALUE CHANGES IN C() BASED ON TOTAL NUMBER OF CLUSTER COMBINATIONS
  axis(side=4)
  ...
  png(paste("//141.20.141.12/SAN_BioGeo/_PERSONAL_FOLDERS/kalamtam/CHAPTERS/Chapter 01/GIS AND MAPPING/ANALYSIS 2023/SOM_clusters/Clustering_May2023/With_Reddy/SOMS_MS_3-6clust/", xdim,"x",ydim,as.character(somSelect),".png",sep=""),height=5000,width=5000,res=500)
  ...
  ### scaled values
  png(paste("//141.20.141.12/SAN_BioGeo/_PERSONAL_FOLDERS/kalamtam/CHAPTERS/Chapter 01/GIS AND MAPPING/ANALYSIS 2023/SOM_clusters/Clustering_May2023/With_Reddy/SOMS_MS_3-6clust/","_scaledvalues",as.character(somSelect),".png",sep=""),height=2000,width=3000,res=300)
  ...
  #Save graph 
  write.csv(resUnits,paste0("//141.20.141.12/SAN_BioGeo/_PERSONAL_FOLDERS/kalamtam/CHAPTERS/Chapter 01/GIS AND MAPPING/ANALYSIS 2023/SOM_clusters/Clustering_May2023/With_Reddy/SOMS_MS_3-6clust/","_resUNits_",as.character(somSelect),".png"))
  ...
  writeRaster(temp_raster,filename=paste0("//141.20.141.12/SAN_BioGeo/_PERSONAL_FOLDERS/kalamtam/CHAPTERS/Chapter 01/GIS AND MAPPING/ANALYSIS 2023/SOM_clusters/Clustering_May2023/With_Reddy/SOMS_MS_3-6clust/",as.character(somSelect),"_output.tif"),format="GTiff",overwrite=T,progress="text")
