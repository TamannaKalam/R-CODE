...
outFolder <- ("//141.20.141.12/SAN_BioGeo/_PERSONAL_FOLDERS/kalamtam/CHAPTERS/Chapter 01/3_FinalAnalysis-April2024/3_SOMS/noMoulds/No Moulds_all years/")
...
combined_plot
ggsave(paste0(outFolder, "3Clusters_trajec_300m_5TC_Nomoulds.png"), combined_plot, width = 12, height = 6, units = "in")
...
writeRaster(temp_raster,filename=paste0(outFolder, "3Clusters_5TC_3000m.tif"),format="GTiff",overwrite=T,progress="text")
...
ggsave(paste0(outFolder, "6-Cluster_trajectories-3km_noMoulds.jpg"),combined_plot, width = 10, height = 12, units = "in")

#[16] Write the SOM raster to drive
ras_df <- SOM_FC_df
coordinates(ras_df) = ~x+y
temp_raster = rasterize(ras_df, stack[[2]], ras_df$cluster) # Write the raster (for QGIS), change [] for different combinations
writeRaster(temp_raster,filename=paste0(outFolder, "6Clusters5TC_noMoulds_3km.tif"),format="GTiff",overwrite=T,progress="text")
...
##################################CODE FOR SHOWING ALL THE PIXEL LINES ###########################
plot <- ggplot(df_long, aes(x = Year, y = Forest_Cover)) +
