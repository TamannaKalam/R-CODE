...
stack_df_filter <- new_stack_df %>%  
  ...
  mutate(FCC_Avg = rowMeans(dplyr::select(., c(X2010_2020, X2000_2010, X1930_X2000, X1880_X1930)))) %>% 
...
stack_df_filter<-stack_df_filter[,c("StudyArea","RD1880","RD1930","EN2000","EN2010","x","y","EN2020","X2010_2020","X2000_2010","X1930_X2000","X1880_X1930","FCC_Avg","MaxGain","MaxLoss")]
...
outFolder <- ("//141.20.141.12/SAN_BioGeo/_PERSONAL_FOLDERS/kalamtam/CHAPTERS/Chapter 01/1. Data_Analysis_Mapping_Forest/ANALYSIS_2024/SOMs-3km/")
...
SOM_FC_df <- SOM_df %>% right_join(stack_df_filter, by=c("x", "y")) %>% dplyr::select(x, y, cluster, RD1880, RD1930, EN2000, EN2010, EN2020)
...
colors <- c("#000004","#316e20", "#ff9f6f","#8c2a81","#ea3fea")
Names <- c("x","High forest cover","Forest loss","Forest expansion","Forest transition")
...
for (cluster_num in cluster_numbers) {
  ...
  cluster_data <- cluster_data[, c("ID", "RD1880","RD1930","EN2000","EN2010","EN2020")]
  ...
  plot <- ggplot(df_long, aes(x = Year, y = Forest_Cover)) +
    geom_line(aes(group = ID), linewidth = 0.01, color = colors[cluster_num]) +# "#097969") +
    ...
    geom_line(aes(x=Year, y = mean_Forest_Cover), color = "black", linetype = "solid", linewidth= 1.5) +
...
combined_plot
ggsave(paste0(outFolder, "5Cluster_trajectories10TC.png"), combined_plot, width = 16, height = 8, units = "in")
...
#colors <- c("#000004","#316e20", "#ff9f6f","#8c2a81","#ea3fea","#c8cbc1")
colors <- c("#000004","#316e20", "#f7901b","#9e6cc2","#f717b8","#c8cbc1")
...
for (cluster_num in cluster_numbers) {
  ...
  plot <- ggplot(df_long, aes(x = Year, y = Forest_Cover)) +
    ...
    theme(
      ...
      axis.line = element_line(color = "black"),
      axis.text.x = element_text(angle = 90, hjust = 1, size = 24, family = "Times New Roman"),
      axis.text = element_text(color = "black", size = 24, family = "Times New Roman"),
      axis.title = element_text(color = "black", size = 24, family = "Times New Roman"),
      axis.title.x = element_text(size = 24, family = "Times New Roman"),  # X-axis label font adjustments
      axis.title.y = element_text(size = 24, family = "Times New Roman"),
      plot.title = element_text(color = "black", size= 24, family = "Times New Roman")
    )
  #[14] Store the plot in the list
  plot_list[[cluster_num]] <- plot
}
...
ggsave(paste0(outFolder, "6-Cluster_trajectories-3km-rearranged.png"), combined_plot_reordered, width = 12, height = 13, units = "in")

#[16] Write the SOM raster to drive [these clusters are in their original sequence and not in re-arranged]
ras_df <- SOM_FC_df
coordinates(ras_df) = ~x+y
...
#Extra 
foo <- SOM_FC_df %>% 
  na.omit() %>% pivot_longer(cols = RD1880:EN2020, names_to = "Year", values_to = "Forest_Cover") %>% 
  mutate(Year = as.numeric(str_sub(Year, start = -4)))  %>%
  group_by(Year,cluster) %>%
  summarize(mean_Forest_Cover = mean(Forest_Cover), sd_Forest_Cover = sd(Forest_Cover)) %>% 
  mutate(Year = as.numeric(Year))

ggplot(foo, aes(x = Year, y = mean_Forest_Cover, color=cluster )) +
  geom_line() +
  geom_ribbon(aes(x = Year, 
                  ymin = pmax(mean_Forest_Cover - sd_Forest_Cover, 0), 
                  ymax = pmin(mean_Forest_Cover + sd_Forest_Cover, 100)), 
...
#---------- TO COUNT HOW MANY PIXELS IN EACH CLUSTER------------

cluster_counts <- table(SOM_FC_df$cluster)
...
for (cluster_num in cluster_numbers) {
  ...
  mean_2000 <- mean(cluster_data$EN2000, na.rm = TRUE)
...
plot_list <- list() # Create an empty list to store the plots
colors <- c("#ff9f6f","#c6ccbf","#ea3fea","#316e20","#8c2a81")
Names <- c("Forest loss","Low forest","Low gain","High forest cover","Forest expansion")
...
coordinates(ras_df) = ~x+y
temp_raster = rasterize(ras_df, stack[[1]], ras_df$cluster) # Write the raster (for QGIS)
writeRaster(temp_raster,filename=paste0(outFolder, "5Clusters10TC.tif"),format="GTiff",overwrite=T,progress="text")

#[15] Combine all plots into one graph by reordering them (eg. loss to gain)
#Define the desired order of clusters
desired_order <- c(2,4,1,3,5)
...
ggsave(paste0(outFolder, "5-Cluster_trajectories-3km-rearranged.png"), combined_plot_reordered, width = 16, height = 14, units = "in")
