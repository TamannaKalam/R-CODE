...
stack_df_filter <- new_stack_df %>%  
  ...
  mutate(FCC_Avg = rowMeans(dplyr::select(., c(X2010_2020, X2000_2010,X1995_X2000,X1985_X1995,X1975_X1985,X1930_X1975,X1880_X1930)))) %>% 
  mutate(MaxGain = pmax(X2010_2020, X2000_2010,X1995_X2000,X1985_X1995,X1975_X1985,X1930_X1975,X1880_X1930)) %>% 
  mutate(MaxLoss = pmin(X2010_2020, X2000_2010,X1995_X2000,X1985_X1995,X1975_X1985,X1930_X1975,X1880_X1930))

stack_df_filter<-stack_df_filter[,c("StudyArea","RD1880","RD1930","RD1975","RD1985","RD1995","EN2000","EN2010","x","y","EN2020","X2010_2020","X2000_2010","X1995_X2000","X1985_X1995","X1975_X1985","X1930_X1975","X1880_X1930","FCC_Avg","MaxGain","MaxLoss")]

#[5] Subset the data by selecting only absolute % change (x2010_2020,X2000_2010..),FCC_Avg, MaxGain & MaxLoss 
stack_df_filter_sub <- stack_df_filter[, c(12:21)] 
stack_matrix = as.matrix(stack_df_filter_sub)

##>>>>>>>>>>>>>>>>>>>>>## PART 2: RUN THE SOMs ##<<<<<<<<<<<<<<<<<<<<<##

#[6] PREPARE THE LIST AND FUNCTION FOR PARALLEL PROCESSING
SOM_combos = as.data.frame(matrix(ncol=4,nrow=4))  #nrow CHANGES BASED ON THE TOTAL NUMBER OF CLUSTER COMBINATIONS YOU CHOOSE
SOM_combos[,1] <- c(3,3,3,5) #CHANGES BASED ON COMBINATIONS CHOSEN
SOM_combos[,2] <- c(1,2,3,2) #CHANGES BASED ON COMBINATIONS CHOSEN
soms_input = list()
...
outFolder <- ("//141.20.141.12/SAN_BioGeo/_PERSONAL_FOLDERS/kalamtam/CHAPTERS/Chapter 01/3_FinalAnalysis-April2024/3_SOMS/")
...
df_xy = stack_df_filter[,9:10]  # CHANGE TO the column numbers of x an y 
...
SOM_FC_df <- SOM_df %>% right_join(stack_df_filter, by=c("x", "y")) %>% dplyr::select(x, y,cluster,RD1880,RD1930,RD1975,RD1985,RD1995,EN2000,EN2010,EN2020)
...
combined_plot
ggsave(paste0(outFolder, "3Cluster_trajectories.png"), combined_plot, width = 10, height = 4, units = "in")
